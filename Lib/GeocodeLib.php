...
//$xmlArray = $res;
$xmlArray['result'] = $res['results'];
$xmlArray['status'] = $res['status'];
} else {
try {
$res = Xml::build($result['body']);
$xmlArray = Xml::toArray($res);
$xmlArray = $xmlArray['GeocodeResponse'];
} catch (Exception $e) {
...
if (isset($xmlArray['result'][0])) {
//$xmlArray['result'] = $xmlArray['result'][0];
$accuracy = $this->_parse('types', $xmlArray['result'][0]);
...
if ($res = $this->HttpSocket->request($url)) {
return $res;
}
